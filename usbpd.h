...
//       Filename: usbpd.h
//------------------------------------------------------------------------------
//       Copyright: Bogdan Ionescu (c) 2025
//------------------------------------------------------------------------------
//       Purpose : USB PD Library
//------------------------------------------------------------------------------
...
//       Notes :
//
//       USB PD Initialisation
//       Start condition:
//        - any CC line is pulled up by source
//       Process:
//       - Source sends a "SOURCE_CAP" every: tTypeCSendSourceCap (100-200ms)
//       -- max response time: tReceive (1.1ms) --
//       - Sink must respond with "GOODCRC"
//
//       -- max response time: tSenderResponse (33ms) --
//
//       - Sink sends a "REQUEST"
//       - Source must respond with "GOODCRC"
//
//       - Source later sends "ACCEPT"
//       - Sink must respond with "GOODCRC"
//
//       - Source even later sends "PS_RDY"
//       - Sink must respond with "GOODCRC"
//
//       - Sink can now send "GET_SOURCE_CAP" anytime
//       - Source must respond with "GOODCRC"
//       - Source will later send "SOURCE_CAP" and the cycle will repeat
//
//------------------------------------------------------------------------------
//      Changelog:
...
#define FIXED_SUPPLY_FMT_ARGS(pdo)                                    \
    ((pdo)->FixedSupply.MaxCurrentIn10mA * 10),                       \
        ((pdo)->FixedSupply.VoltageIn50mV * 50),                      \
        ((pdo)->FixedSupply.PeakCurrent),                             \
        ((pdo)->FixedSupply.EPRModeCapable ? "Yes" : "No"),           \
...
#define VARIABLE_SUPPLY_FMT_ARGS(pdo)                  \
    ((pdo)->VariableSupply.MaxCurrentIn10mA * 10),     \
        ((pdo)->VariableSupply.MinVoltageIn50mV * 50), \
...
#define BATTERY_SUPPLY_FMT_ARGS(pdo)                  \
    ((pdo)->BatterySupply.MaxPowerIn250mW * 250),     \
        ((pdo)->BatterySupply.MinVoltageIn50mV * 50), \
...
#define SPR_PPS_FMT_ARGS(pdo)                     \
    ((pdo)->SPR_PPS.MaxCurrentIn50mA * 50),       \
        ((pdo)->SPR_PPS.MinVoltageIn100mV * 100), \
        ((pdo)->SPR_PPS.MaxVoltageIn100mV * 100), \
        ((pdo)->SPR_PPS.PPSpowerLimited ? "Yes" : "No")
...
#define EPR_AVS_FMT_ARGS(pdo)                     \
    ((pdo)->EPR_AVS.PDPIn1W),                     \
        ((pdo)->EPR_AVS.MinVoltageIn100mV * 100), \
        ((pdo)->EPR_AVS.MaxVoltageIn100mV * 100), \
        ((pdo)->EPR_AVS.PeakCurrent)
...
#define SPR_AVS_FMT_ARGS(pdo)                         \
    ((pdo)->SPR_AVS.MaxCurrent15To20VIn10mA * 10),    \
        ((pdo)->SPR_AVS.MaxCurrent9to15VIn10mA * 10), \
...
static_assert(sizeof(USBPD_SPR_CapabilitiesMessage_t) == (7 * sizeof(uint32_t)), "USBPD_SPR_CapabilitiesMessage_t size mismatch");
...
typedef struct
{
    uint32_t OperatingCurrentIn50mA : 7u;
    uint32_t Reserved_7_8bit : 2u;       // shall be set to zero
    uint32_t OutputVoltageIn100mV : 12u; // NOTE: Output voltage in 25mV units, the least two significant bits Shall be set to zero making the effective voltage step size 100mV.
    uint32_t Reserved_21bit : 1u;        // shall be set to zero
    uint32_t ERPCapable : 1u;
    uint32_t UnchunkedExtendedMessage : 1u;
    uint32_t NoUSBSuspended : 1u;
...
bool USBPD_IsPPS(const USBPD_SourcePDO_t *pdo)
{
    return (pdo->Header.PDOType == eUSBPD_PDO_AUGMENTED) &&
           (pdo->Header.AugmentedType == eUSBPD_APDO_SPR_PPS);
}
